...
/*   Updated: 2023/03/29 00:47:14 by absalhi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cub_rect_put(t_game *g, t_iterators pos, int color, int size)
{
	...
	while (++it.i < horizontal_size)
	{
		...
		while (++it.j < vertical_size)
			if (pos.i * horizontal_size + it.i < g->win.height
				&& pos.j * vertical_size + it.j < g->win.width
				&& (it.i == 0 || it.i == horizontal_size - 1
					|| it.j == 0 || it.j == vertical_size - 1))
				cub_pixel_put(g, vertical_size * pos.j + it.j,
					horizontal_size * pos.i + it.i + 1, color);
	}
}
